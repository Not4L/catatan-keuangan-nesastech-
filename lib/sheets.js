...
export async function resetSaldoHarian() {
  ...
  const sudahJalanHariIni = rows.some(r => {
    if (!r[1]) return false;
    const tglRow = excelSerialToDate(Number(r[1])).toISOString().slice(0, 10);
    return r[3] === 'Setoran' && tglRow === hariIni;
  });
  if (sudahJalanHariIni) return { skipped: true };
